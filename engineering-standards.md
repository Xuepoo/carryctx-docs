...
**当前架构：** CarryCtx v0.9.x 使用 Rust 2024、Cargo、`rusqlite` 和原生 CLI 二进制，主要通过 Cargo、GitHub Releases 及平台包分发；npm 仅为可选 wrapper 分发渠道。

> **历史范围：** 本文保留的 TypeScript/Bun、`bun:sqlite`、`package.json` 和 npm-first CLI 内容属于 v0.1 设计记录，仅用于解释历史决策，不适用于 CarryCtx v0.9.x.
...
## 5.2 当前 v0.9.0 Workspace（4+1 Crates，002 完成）
...
├── src/                       # 根 `carryctx` thin facade: re-export carryctx-cli/cli-core (+ pack/vcs)，仅保留 src/main.rs + src/commands/（二进制入口，follow-on 迁至 carryctx-cli）+ src/lib.rs facade
├── migrations/project/        # SQL 源码；编译期通过 carryctx-sqlite 嵌入（include_str!）
└── tests/
```

> **0.9.1 src 去重（CTX-0128）：**根 `src/adapter/`、`src/application/`、`src/domain/`、`src/repository/`、`src/error.rs`、`src/output.rs`（与 `crates/carryctx-cli/src/` 字节一致的重复实现）已删除，`src/lib.rs` 改为纯 facade（`pub use carryctx_cli::{adapter,application,domain,error,output,repository}` + `carryctx_core::{domain,error,repository as core_*}` + `pack`/`vcs`透传 + `pub use carryctx_cli as cli`）。仅 `src/main.rs` + `src/commands/` 留在根 crate（二进制入口，follow-on 迁至 `carryctx-cli`）。
>
...
v0.9.0 的实现使用 Rust 模块和 Cargo crate；SQLite adapter 通过
...
以上 `bun:test` 和目录约定仅适用于 v0.1 TypeScript 实现。v0.9.0 使用
...
CI 要求。v0.9.0 的发布 workflow 使用 `cargo build --release --locked` 构建各平台
...
以上 Hook/CI 命令属于 v0.1 工具链记录。v0.9.0 的最低验证基线是
...
以上 npm 包内容和 `just release-check` 流程仅为 v0.1 历史记录。v0.9.0 发布以
