...
不会。CarryCtx 设计为状态缓存，而非永久归档。进度条目 (`progress`) 和快照 (`checkpoints`) 主要保留给当前版本。随着任务 `complete`，它的中间繁杂细节后续可以通过归档命令定期清理，保证 `<git-common-dir>/carryctx/state.sqlite` 始终轻量高效。
