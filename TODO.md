...
- [ ] Implement the version-metadata CI check specified in
      ...
      known `use-carryctx` drift (skill frontmatter `1.1.0` /
      `min_carryctx 0.9.1` versus a README that still claims a `v0.8.0`
      surface — now `v0.9.1`; merge-milestone contract values: ctxpack
      `format_version` `2`, DB schema `18`).
...
## ctxpack hardening (merge semantics landed on main, unreleased)
