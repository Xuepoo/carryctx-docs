...
**文档版本：** v0.9.1
**适用版本：** CarryCtx v0.9.1 / v0.9.x（merge milestone 的合并 staging 与快照缓存为 Unreleased）
...
merge milestone（Unreleased）未引入 `[snapshot]` 或 `[merge]` 配置键：snapshot
...
（Unreleased），`snapshots/<export_id>/` 由用户手工放置 bundle 以支持离线
...
- `import --mode merge` / `conflict apply` 的原子交换（验证过的 pre-merge
  backup，路径在成功信封的 `preMergeBackupPath` 返回；Unreleased）
