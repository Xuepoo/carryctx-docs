...
## Where we are (v0.9.1; merge milestone merged to main, unreleased)
...
- Merge milestone, merged to `main` pending release: ctxpack v2 (`parents`
  DAG, `tombstones`, `redacted` flag), schema 18, `import --mode merge`
  with conflict staging and `conflict list/show/resolve/apply/abort`, and
  local snapshot Git refs (`export --snapshot`, `import --from-git`).
  In flight: merge-snapshot commits after `--mode merge` (CTX-0145), the
  three-way git e2e matrix (CTX-0146), the local-only ref guard (DEC-0052,
  PR #154), and this docs sync (CTX-0147).
...
3. **ctxpack hardening.** Export profiles and privacy review (hostname,
   ...
   diff/inspection UX. The semantic merge milestone is landed on main; its
   remaining rollout items are merge-snapshot commits, the git e2e matrix,
   and the local-only snapshot-ref guard (DEC-0052).
