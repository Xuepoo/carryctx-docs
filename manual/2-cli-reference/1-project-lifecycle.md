...
CarryCtx v0.9.1 是面向 Agent 与人类协作者的、local-first 的**全项目生命周期持久化与控制层**。
...
v0.9.x 不提供通用的 Completion Gates 或已发布的 Automation Engine。
...
9. **发布证据**：发布前由外部 harness 执行测试、Lint、构建和其他验证，并将结果、commit、已知缺口
   ...
   审计查询，但不替代验证工具或发布流程。
10. **状态交换与合并**（merge milestone，已合入 main，尚未发布）：用
    `carryctx export --pack-format dir -o <dir>` 生成 ctxpack（v1 现行，v2 为
    Unreleased：`parents` DAG、`tombstones`、`redacted` 标记），用
...
merge milestone（Unreleased）的合并 staging 位于 `<git-common-dir>/carryctx/merges/`，快照 DAG
...
carryctx import ./ctxpack-dir/ --mode merge             # 三方合并（Unreleased）
```
