...
- **Re-anchor policy (normative):** `projects.repository_root/git_common_dir` are rewritten to the target. `worktrees.*` rows whose `normalized_path` does not exist at the target are dropped from the live table with a warning (visible in `doctor` as pruned, audited as `worktree.pruned`); `sessions.working_directory` values are kept as history but never trusted for liveness. Absolute paths are never compared for equality across machines.
  - **Pruned/absent worktree references (normative):** the loader inserts `worktrees` before `sessions`/`checkpoints` (both reference `worktrees.id`). A `sessions.worktree_id` or `checkpoints.worktree_id` that names a worktree not live at the target — pruned by the rule above, or absent from the bundle — is set to `NULL` on insert; the history row itself is kept, and the import result carries one aggregated `warnings` entry per affected table. Every other foreign key stays strictly enforced, so a genuinely inconsistent bundle still fails closed.
  - The nulling decision is import-time only because whether a worktree path exists is a property of the import machine, not of the bundle: export stays lossless and keeps the original `worktree_id` values. This mirrors the dangling-`events.task_id` convergence already used by the loader (see `design/2026-09-10-mergeable-git-managed-state.md` §1.7).
  - **Implementation status (2026-09-11):** merged to `carryctx-cli` main by CTX-0137 (PR #132, `a5f6e2e`) and released in `carryctx` 0.10.0 (tag `v0.10.0`; GitHub release <https://github.com/Xuepoo/carryctx/releases/tag/v0.10.0>, published 2026-09-10), so this behavior is shipped.
